...
! function() {
    ...
    function b() {
        for (var a = g, b = document.getElementsByTagName("script"), c = b.length; c--;)
    ...
    function c(a, b, c) {
        var d = c ? c : "";
        return '<iframe id="' + a.videoid + '" width="' + a.width + '" height="' + a.height + '" src="https://www.youtube.com/embed/' + d + "?enablejsapi=" + a.enablejsapi + "&autoplay=" + a.autoplay + "&cc_load_policy=" + a.ccloadpolicy + "&color=" + a.color + "&controls=" + a.controls + "&disablekb=" + a.disablekb + "&end=" + a.end + "&fs=" + a.fs + "&hl=" + a.hl + "&iv_load_policy=" + a.ivloadpolicy + "&playlist=" + b.playlistArray + "&playsinline=" + a.playsinline + "&rel=" + a.rel + "&showinfo=" + a.showinfo + "&start=" + a.start + "&modestbranding=" + a.modestbranding + "&loop=" + a.loop + "&listType=" + a.listType + "&list=" + a.list + "&origin=" + a.origin + '" frameborder="0" allowfullscreen></iframe>'
    }
    ...
    e.service("ngYoutubeEmbedService", ["$window", "$rootScope", function(a, b) {
    ...
    }]), e.directive("ngYoutubeEmbed", ["$sce", "ngYoutubeEmbedService", "$rootScope", function(c, e, g) {
        return {
            restrict: "E",
            ...
            link: function(h, i, j) {
                ...
                if (k.enablejsapi && !b() && (a(), e.setReadyState()), k.enablejsapi && g.$on("youtubeIframeEmbedApiLoaded", function(a, b) {
                        if (b === k.videoid) {
                            var c = new YT.Player(b, {
                                events: {
                                    onReady: h.onready,
                                    onStateChange: h.onstatechange,
                                    onPlaybackQualityChange: h.onplaybackqualitychange,
                                    onPlaybackRateChange: h.onplaybackratechange,
                                    onError: h.onerror,
                                    onApiChange: h.onapichange
                                }
                            });
                            g.$emit("addNewPlayer", {
                                player: c,
